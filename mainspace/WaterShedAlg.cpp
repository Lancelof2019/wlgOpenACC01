...
Array2D<int>shouldBeZeroImage(rows,cols,0);
Bool2D visBool(rows,cols);

Array2D<int>threshMat(rows,cols,0);
Array2D<int>antiMat(rows,cols,0);
Array2D<int>distanMat(rows,cols,0);



/*
#pragma acc enter data copyin(threshmat,markers,visArr,plots,markerMap,inprioq,temp,nextSet,shouldBeZeroImage,visBool,threshmat.matImg[:rows][:cols],markers.matImg[:rows][:cols],visArr.matImg[:rows][:cols],plots.matImg[:rows][:cols],markerMap.matImg[:rows][:cols],inprioq.matImg[:rows][:cols],temp.matImg[:rows*cols][:3],nextSet.matImg[:rows*cols*9][:3],shouldBeZeroImage.matImg[:rows][:cols],visBool.boolarr[:rows][:cols])

*/
//#pragma acc enter data copyin(visBool,visBool.boolarr[:rows][:cols])
...
//int **shouldBeZeroImage=new int*[rows];
//for(int i=0;i<rows;)
...
/****Buffer allocation****/
   ...
   clock_t start1,end1;
    ...
    start=omp_get_wtime();
     ...
     walg->processImage(srcImg,duplImage,threshmat,markers,visArr,plotx,ploty,plots,inprioq,markerMap,temp,nextSet,arr4D,mat4D,visBool,shouldBeZeroImage,threshMat,antiMat,distanMat);
     end=omp_get_wtime();
     end1=clock();
