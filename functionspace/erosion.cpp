...
Mat WatershedAlg::erosion(Mat &image, int **kernel,int &kernelRows,int &kernelCols,Array2D<int>&shouldBeZeroImage) {
   ...
   // vector< vector < int> > shouldBeZeroImage(drows, vector<int>(dcols, 0));	
	#pragma acc enter data copyin(dillstart[:drows*dcols],shouldBeZeroImage,shouldBeZeroImage.matImg[:drows][:dcols],kernel[:n][:m])
...
//	     #pragma acc enter data copyin(dillstart[:drows*dcols],shouldBeZeroImage[:drows][:dcols])
        ...
        #pragma acc exit data delete (shouldBeZeroImage,shouldBeZeroImage.matImg[:drows][:dcols],kernel[:n][:m],dillstart[:drows*dcols])
        return dill;
